...
func loadMonitorSnapshotFromClient(ctx context.Context, client kubernetes.Interface, checkedAt string) monitorSnapshot {
	...
	snapshot.events, snapshot.eventsErr = listMonitorWarningEvents(ctx, client)
...
// listMonitorWarningEvents fetches Warning events for health checks. It filters
// server-side via FieldSelector so the summary/warning-events check sees the
...
func buildMonitorAIContext(issue MonitorIssue, events []MonitorEvent, logs []MonitorLogPreview) MonitorAIContext {
	...
	if issue.RelatedEventCount > 0 {
		aiContext.Signals = append(aiContext.Signals, fmt.Sprintf("%d related Kubernetes events found.", issue.RelatedEventCount))
	}
	for _, event := range limitMonitorEvents(events, 5) {
		aiContext.Evidence = append(aiContext.Evidence, fmt.Sprintf("Event %s/%s: %s", event.Type, event.Reason, event.Message))
	}
	for _, log := range logs {
		if log.Error != "" {
			aiContext.Evidence = append(aiContext.Evidence, fmt.Sprintf("Log preview %s previous=%t failed: %s", log.Container, log.Previous, log.Error))
			continue
		}
		aiContext.Evidence = append(aiContext.Evidence, fmt.Sprintf("Log preview %s previous=%t contains %d bytes.", log.Container, log.Previous, len(log.Content)))
	}
	return aiContext
}
